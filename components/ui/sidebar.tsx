...
>(({ side = "left", variant = "sidebar", collapsible = "offcanvas", className, children, ...props }, ref) => {
    ...
    return (
      <Sheet open={openMobile} onOpenChange={setOpenMobile} {...props}>
        <SheetContent
          ...
          data-mobile="true"
          side="left"
          className="w-[--sidebar-width] border-r border-sidebar-border/70 !bg-sidebar p-0 text-sidebar-foreground [&>button]:hidden"
          style={
            {
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          "duration-200 fixed inset-y-0 z-[5] hidden h-svh transition-[left,right,width] ease-linear md:flex",
          state === "expanded" ? "w-[--sidebar-width]" : collapsible === "icon" ? "w-[--sidebar-width-icon]" : "w-[--sidebar-width]",
          side === "left"
            ? "left-0"
            : "right-0",
          ...
          variant === "floating" || variant === "inset"
            ? collapsible === "icon" && state === "collapsed" && "p-2"
      ...
      >
        <div
          ...
          data-state={state}
          data-collapsible={collapsible}
          className="flex h-full w-full flex-col bg-[linear-gradient(180deg,hsl(var(--sidebar-background))_0%,hsl(var(--sidebar-background))_72%,hsl(var(--sidebar-primary)/0.05)_100%)] group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:border-sidebar-border group-data-[variant=floating]:shadow"
        >
          {children}
        </div>
      </div>
...
const SidebarRail = React.forwardRef<HTMLButtonElement, React.ComponentProps<"button">>(({ className, ...props }, ref) => {
  ...
  return (
    <button
      ...
      className={cn(
        "absolute inset-y-0 z-10 hidden w-4 -translate-x-1/2 transition-all ease-linear after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] after:-translate-x-1/2 after:bg-sidebar-border after:opacity-0 after:transition-opacity after:duration-200 hover:after:opacity-100 group-data-[side=left]:-right-4 group-data-[side=right]:left-0 sm:flex",
  ...
  return (
    <main
      ...
      className={cn(
        ...
        "ml-0 md:ml-[calc(var(--sidebar-width)+1rem)]",
        state === "collapsed" && "md:ml-[calc(var(--sidebar-width-icon)+1rem)]",
        className
      )}
      {...props}
    />
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex w-full items-center gap-2 overflow-hidden rounded-xl p-2 text-left text-sm outline-none ring-sidebar-ring transition-[width,height,padding,background-color,box-shadow,border-color] duration-200 hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-[[data-sidebar=menu-action]]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-sidebar-accent data-[active=true]:font-medium data-[active=true]:text-sidebar-accent-foreground data-[state=open]:hover:bg-sidebar-accent data-[state=open]:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:!size-10 group-data-[collapsible=icon]:!p-2 group-data-[collapsible=icon]:justify-center group-data-[collapsible=icon]:!gap-0 [&>span]:truncate group-data-[collapsible=icon]:[&>span]:hidden [&>svg]:size-4 [&>svg]:shrink-0 group-data-[collapsible=icon]:[&>svg]:mx-auto",
  {
    variants: {
      variant: {
